...
description: Use for Vue 3, .vue files, Vue Router, or Pinia. Skip when the repo has no Vue.
license: MIT
metadata:
...
Load only when the working tree has Vue: `.vue` files, `vue` in package.json, or the user named Vue. Do not convert a React or static page to Vue to apply this skill.

If the project uses Options API, stay on Options API. Do not invent sibling skills `vue-options-api-best-practices` or `vue-jsx-best-practices`. We do not ship them.
