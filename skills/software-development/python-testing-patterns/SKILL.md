...
pytest, fixtures, mocks, and parameterization. Pair with `python-best-practices` and `tdd` when the user asked for TDD.

## Local constraints

- Invoke tests with `uv run pytest`. Do not tell the repo to `pip install pytest`.
- Do not rewrite a test to make it green. Fix the code or the fixture.
- Mock only what cannot run locally. Prefer the real function, the real DB in a test container, or the project's own test harness.
- Coverage percent is not the goal. Cover the behavior the ticket named.
- `tdd` stays the TDD skill. This one is patterns inside pytest.
...
# BAD here. Use the project runner.
