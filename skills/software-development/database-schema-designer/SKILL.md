...
description: Design Postgres schemas, indexes, and reversible migrations.
license: MIT
---
...
Default dialect is Postgres. MySQL examples below are translation material, not the target.

- Primary keys: `BIGINT GENERATED ALWAYS AS IDENTITY`, or UUID only when global opacity is required.
- Time: `TIMESTAMPTZ`. Money: `NUMERIC`. Strings: `TEXT` unless a real length bound exists.
- Index every foreign key. Postgres does not do this for you.
- Expand/contract for live tables: add nullable, backfill, then constrain. Do not add `NOT NULL` onto populated rows in one step.
- After a draft schema, load `supabase-postgres-best-practices` for query, lock, and type rules.
- NoSQL only if the repo already uses it. Do not introduce Mongo or Dynamo to a Postgres service.
- Do not invent a second database for a problem the current one can hold.
