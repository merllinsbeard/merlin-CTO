...
This is a local copy. Upstream points at sibling skills we do not ship.

- Live failure now: `systematic-debugging` or `diagnosing-bugs`, then `production-release-verification` if the question is revision/rollback.
- Measured slowness: profile first. Do not add a metrics vendor to find a hot function.
- Default stack here is Docker + Python + Telegram, not Express + Prometheus. Structured logs with a request id come first. OpenTelemetry or Prometheus only if the repo already has them.
- Never log tokens, passwords, raw prompt bodies, or full PII.
- Do not invent dashboards or SaaS. Use what the repo already emits.
...
**NOT for:**
- Diagnosing a failure happening right now. That is `systematic-debugging`.
- Profiling measured slowness. Measure first, then change the hot path.
- Launch checklists and rollback. That is `production-release-verification`.
...
**Never log secrets, tokens, passwords, or full PII.** Telemetry pipelines leak. Allowlist fields. Do not log whole request bodies or model prompts.
