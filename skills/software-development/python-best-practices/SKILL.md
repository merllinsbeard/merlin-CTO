...
Language-specific idioms on top of `principle-type-system-discipline` and `principle-boundary-discipline`.

## Local constraints

- Run with `uv` / `uv run`. Do not add `pip install` to a repo that already has `uv`.
- Tests: `python-testing-patterns`. Do not rewrite a test to make it green.
- Logging: structured fields, not interpolated prose. See `observability-and-instrumentation`.
- This skill is not a React or TypeScript guide.
