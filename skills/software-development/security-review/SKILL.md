...
Identify exploitable security vulnerabilities in **our** code. Report only **HIGH CONFIDENCE** findings: a vulnerable pattern plus attacker-controlled input.

## Local constraints

- Review only the file, diff, or service the user named. Do not scan the internet or a third-party product.
- Do not write exploits, PoCs, payloads, or attack steps. Name the sink, the source, and the fix.
- Prompt injection, tool-call blast radius, and secrets in logs count. They live in `references/modern-threats.md` and `references/logging.md`.
- Auth on a path is not a reason to skip an IDOR. Note the auth requirement, then still check object-level access.
- This is not `web-pentest`. Do not probe live systems you do not operate.
...
- **Impact**: [What breaks for a real user or tenant]
- **Evidence**:
  ```python
  [Vulnerable code snippet]
  ```
- **Fix**: [How to remediate. No exploit payload.]
