...
description: Use for delegate_task, Kanban, and all agent-facing text.
version: 0.1.0
...
metadata:
  hermes:
    tags: [prompts, agents, skills, documentation]
    related_skills: []
---

Reference for writing any text another agent will read or execute. The packaging differs; the writing does not: the same levers make each one predictable — the agent taking the same _process_ every run, not producing the same output.

## When to Use

- Before calling `delegate_task` or sending a prompt to a CLI agent.
- When writing Kanban cards, goal prompts, briefs, specs, tickets, handoffs, or work orders for agents.
- When creating or editing skills, `AGENTS.md`, `CLAUDE.md`, or docs reached by a context pointer.

Do not use it for text intended only for a human.
